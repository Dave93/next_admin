...
const MobilePushEvents = () => {
  ...
  const dropProps = {
    ...
    customRequest: async (options: RcCustomRequestOptions) => {
      ...
      formData.append('parent', 'mobile_push_events')
  ...
  const { data, isLoading, error } = useQuery(
    ...
    async () => {
      const {
        data: { data: result },
      } = await axios.get(`${webAddress}/api/mobile_push_events`)
      return result
    },
    {
  ...
  const onFinish = async (values: any) => {
    ...
    if (editingRecord) {
      ...
      await axios.put(
        `${webAddress}/api/mobile_push_events/${editingRecord?.id}`,
        {
          ...editingRecord,
          ...values,
        }
      ...
      await axios.post(`${webAddress}/api/mobile_push_events/`, {
        ...values,
      })
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'title',
      render: (_: any, record: any) => {
        return <span dangerouslySetInnerHTML={{ __html: record.title }}></span>
      },
    },
    {
...
MobilePushEvents.displayName = 'MobilePushEvents'
export default MobilePushEvents
